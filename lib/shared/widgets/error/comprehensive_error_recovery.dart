...
/// Comprehensive error recovery widget with all error handling scenarios
class ComprehensiveErrorRecovery extends StatefulWidget {
  final dynamic error;
  ...
  final bool showRefreshOption;
  final bool isCompact;
  final int maxRetries;
  final bool monitorConnectivity;

  const ComprehensiveErrorRecovery({
    super.key,
    required this.error,
    this.operationName,
    this.onRetry,
  ...
  @override
  State<ComprehensiveErrorRecovery> createState() =>
      _ComprehensiveErrorRecoveryState();
}

class _ComprehensiveErrorRecoveryState
    extends State<ComprehensiveErrorRecovery> {
  ...
  void _monitorSystemStatus() {
    ...
    _statusSubscription = ConnectivityService().statusStream.listen((status) {
      if (mounted) {
        ...
        // Show recovery notification when connection is restored
        if (status == ConnectivityStatus.connected &&
            _categorizeError(widget.error) == ErrorCategory.network) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                'Connection restored! You can retry your operation.',
              ),
  ...
  Widget _buildFullErrorRecovery() {
    ...
    return Column(
      children: [
        ...
        // Error display
        widgets.AnimatedErrorDisplay(
          title: _getErrorTitle(errorCategory),
          message: errorMessage,
          icon: _getErrorIcon(errorCategory),
          type: _convertErrorDisplayType(_getErrorDisplayType(errorCategory)),
        ...
        // Action buttons
        const SizedBox(height: 16),
        _buildFullActions(errorCategory),
      ],
    );
  }
  ...
  Widget _buildCompactActions(ErrorCategory errorCategory) {
    ...
    switch (errorCategory) {
      case ErrorCategory.network:
        ...
        if (widget.showRefreshOption && widget.onRefresh != null) {
          actions.add(
            _buildCompactActionButton(
              'Refresh',
      ...
      case ErrorCategory.data:
        if (widget.showClearCacheOption && widget.onClearCache != null) {
          actions.add(
            _buildCompactActionButton(
              'Clear Cache',
  ...
  Widget _buildFullActions(ErrorCategory errorCategory) {
    final actions = <Widget>[];
    ...
    if (actions.isNotEmpty && widget.onRetry != null) {
  ...
  Future<void> _handleRetry() async {
    ...
    try {
      // Check system status before retrying
      ...
      // Show success feedback if retry succeeds
      if (mounted) {
        _showOperationCompletedWithRetries(
          context: context,
          operationName: widget.operationName ?? 'Operation',
          retryCount: _retryAttempts - 1,
        );
      }
    } catch (e) {
      ...
      if (mounted) {
        ErrorHandler.showErrorSnackBar(
          context: context,
          message: 'Retry failed: ${_getUserFriendlyMessage(e)}',
        );
      }
    } finally {
      if (mounted) {
  ...
  String _getRetryButtonText() {
    ...
    if (_retryAttempts > 0) return 'Try Again';
    return 'Retry';
  }
  ...
  ErrorDisplayType _getErrorDisplayType(ErrorCategory category) {
    switch (category) {
      case ErrorCategory.network:
  ...
  /// Show operation completed with retries feedback
  void _showOperationCompletedWithRetries({
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('$operationName completed after $retryCount retries'),
        backgroundColor: Colors.green,
      ),
    );
  }
  ...
  widgets.ErrorDisplayType _convertErrorDisplayType(ErrorDisplayType type) {
    switch (type) {
      case ErrorDisplayType.fullScreen:
        return widgets
            .ErrorDisplayType
            .error; // Default to error for fullScreen
...
/// Error recovery widget with automatic retry
class AutoRetryErrorRecovery extends StatefulWidget {
  final dynamic error;
  final String? operationName;
  final Future<void> Function() operation;
  final int maxAutoRetries;
  final Duration retryDelay;
  final VoidCallback? onFinalFailure;

  const AutoRetryErrorRecovery({
    super.key,
    required this.error,
  ...
  @override
  State<AutoRetryErrorRecovery> createState() => _AutoRetryErrorRecoveryState();
}

class _AutoRetryErrorRecoveryState extends State<AutoRetryErrorRecovery> {
  ...
  Future<void> _startAutoRetry() async {
    if (_autoRetryCount >= widget.maxAutoRetries) {
      widget.onFinalFailure?.call();
      return;
    }
  ...
  void _showOperationCompletedWithRetries({
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          retryCount > 0
              ? '$operationName completed after $retryCount retries'
  ...
  Widget build(BuildContext context) {
    return ComprehensiveErrorRecovery(
      ...
      onRetry: _isAutoRetrying ? null : () => _startAutoRetry(),
      isCompact: false,
